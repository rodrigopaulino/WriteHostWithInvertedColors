function Write-Host {
    ...
    $ForegroundColor = if ($Null -eq $ForegroundColor) { if ($IsWinOS) { [ConsoleColor]::ForegroundColor } else { "White" } }
    $BackgroundColor = if ($Null -eq $BackgroundColor) { if ($IsWinOS) { [ConsoleColor]::BackgroundColor } else { "Black" } }
    if ($InvertColors) {
